...
describe('the short-video term', () => {
  /**
   * The signal the model cannot see. What goes to the engine is the title, the channel,
   * the tags and the description, so length is not something it can learn however often
   * it is told — twenty-eight ratings averaging 1.2 against 3.3 for anything longer, on
   * an installation where four videos in five were short.
   */
  it('is charged against a video under the threshold', () => {
    const scored = scoreCandidate(
      { video: video({ durationSeconds: 45 }), channel: channel(), lane: 'related' },
      context(),
    )
    expect(scored.breakdown.shortPenalty).toBe(DEFAULT_SETTINGS.weights.shortPenalty)
  })

  it('treats the threshold as inclusive, and anything past it as long', () => {
    // YouTube's line is "up to three minutes", so 3:00 exactly is a Short.
    expect(
      scoreCandidate(
        { video: video({ durationSeconds: 180 }), channel: channel(), lane: 'related' },
        context(),
      ).breakdown.shortPenalty,
    ).toBe(DEFAULT_SETTINGS.weights.shortPenalty)

    for (const durationSeconds of [181, 900]) {
      const scored = scoreCandidate(
        { video: video({ durationSeconds }), channel: channel(), lane: 'related' },
        context(),
      )
      expect(scored.breakdown.shortPenalty).toBe(0)
    }
  })

  it('is not charged when the length is unknown', () => {
    // Missing evidence, not evidence of shortness. Charging here would demote videos
    // for having been fetched badly.
    const scored = scoreCandidate(
      { video: video({ durationSeconds: null }), channel: channel(), lane: 'related' },
      context(),
    )
    expect(scored.breakdown.shortPenalty).toBe(0)
  })

  it('leaves a short video the model likes above a long one it does not', () => {
    // The reason this is a weight and not a filter: a prediction strong enough still
    // wins, and someone who wants a particular short video is not overruled by a rule.
    ...
    const short = scoreCandidate(
      { video: video({ id: 'youtube:short', durationSeconds: 30 }), channel: channel(), lane: 'related' },
      shared,
    )
    const long = scoreCandidate(
      { video: video({ id: 'youtube:long', durationSeconds: 1800 }), channel: channel(), lane: 'related' },
      shared,
    )
    expect(short.score).toBeGreaterThan(long.score)
  })

  it('can be turned off, or turned into a preference for short videos', () => {
    const off = context({
      settings: { ...DEFAULT_SETTINGS, weights: { ...DEFAULT_SETTINGS.weights, shortPenalty: 0 } },
    })
    expect(
      scoreCandidate({ video: video({ durationSeconds: 30 }), channel: channel(), lane: 'related' }, off)
        .breakdown.shortPenalty,
    ).toBe(0)

    // Negative is a bonus: disliking shorts is this reader's preference, not the
    // system's opinion about what a video should be.
    const preferred = context({
      settings: { ...DEFAULT_SETTINGS, weights: { ...DEFAULT_SETTINGS.weights, shortPenalty: -1.5 } },
    })
    const scored = scoreCandidate(
      { video: video({ durationSeconds: 30 }), channel: channel(), lane: 'related' },
      preferred,
    )
    const longer = scoreCandidate(
      { video: video({ durationSeconds: 1800 }), channel: channel(), lane: 'related' },
      preferred,
    )
    expect(scored.score).toBeGreaterThan(longer.score)
  })

  it('respects a threshold the reader has moved', () => {
    const tenMinutes = context({ settings: { ...DEFAULT_SETTINGS, shortThresholdSeconds: 600 } })
    const scored = scoreCandidate(
      { video: video({ durationSeconds: 400 }), channel: channel(), lane: 'related' },
      tenMinutes,
    )
    expect(scored.breakdown.shortPenalty).toBe(DEFAULT_SETTINGS.weights.shortPenalty)
  })
