...
パスを `api/engine` に絞った Access アプリケーションを別に作り、
そこに **Service Auth** ポリシー（Include: Any Access Service Token）を付ける。
ランナーは `CF-Access-Client-Id` / `CF-Access-Client-Secret` を送り、エッジで検証される。

そのうえで Worker も `Authorization: Bearer <ENGINE_PULL_TOKEN>` を検証する。二層にする理由は、
Access アプリのパス指定を間違えたときに、ルートが開いたままにならないようにするため。

- トークンは32バイトのランダム値（base64url）
- 比較は定数時間で行う
- 失敗時は理由を返さず 403

**当初は Bypass ＋ 共有シークレットで設計していた**（ダッシュボード作業を減らす判断）。
Bypass は Access が認証を一切しない設定なので、保護範囲に穴を開けることになり、
claim のレスポンスに含まれる動画のタイトルと説明文が、トークン漏洩時に読まれる形だった。

サービストークンに切り替えたことでその穴は無くなった。認証されないリクエストは Worker に届かない。

> ⚠️ **重要:** Access アプリケーションは**パスで分ける**こと。
> `yt.mantaroh.com` 全体を対象にしたアプリに Service Auth を足すと、
> サービストークンでサイト全体に到達できてしまう。

engine ルートは3つだけに限定してある。
...
認証情報はリポジトリ直下の `engine-credentials.env`（git 管理外）から読む。環境変数が優先される。
引数にしないのは、履歴とプロセス一覧に残るため。

| キー | 用途 |
|---|---|
| `ENGINE_PULL_TOKEN` | Worker が検証するトークン |
| `CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET` | エッジの Access を通るサービストークン |
...
| Bypass ＋ 共有シークレット | 当初はこちらを選んだが、Access の保護範囲に穴が開く。サービストークンなら穴が無く、手間の差は小さい |
...
**パスを `api/engine` に絞った Access アプリケーションと、Service Auth ポリシー、サービストークンの発行が必要**（ダッシュボード作業）。
...
| フェーズ | 予定ターン数 | 実際のターン数 |
|---|---:|---:|
| 設計（本ドキュメント） | 2 | 3（認証方式の差し戻しを含む） |
| 実装 | 未定 | 1（設計承認後の1ターンで完了） |

実装で足したもの: Worker 側18テスト、Python 側10テスト。

設計時に読み違えていたのは既存関数のシグネチャ2つ（`activateModel` と `appendRating`）だけ。

方式は一度変えた。認証を Bypass ＋ 共有シークレットで設計していたが、
ダッシュボードでの設定中に Access の穴を避けるサービストークン方式へ切り替えた。
最初から両案を並べて比較していれば、この差し戻しは避けられた。
